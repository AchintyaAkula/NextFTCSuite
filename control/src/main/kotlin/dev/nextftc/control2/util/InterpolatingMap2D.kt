...
class InterpolatingMap2D(
    ...
    constructor(
        ...
        require(values.size == yKeys.size) { "values.size (${values.size}) != yKeys.size (${yKeys.size})" }
    ...
    operator fun get(x: Double, y: Double): Double {
        ...
        val x1Entry1 = xRow1.ceilingEntry(x) ?: throw NoSuchElementException("No entry with x >= $x")
        ...
        val tx = if (x0Entry.key == x1Entry.key) 0.0 else (x - x0Entry.key) / (x1Entry.key - x0Entry.key)
        val ty = if (y0Entry.key == y1Entry.key) 0.0 else (y - y0Entry.key) / (y1Entry.key - y0Entry.key)
    ...
    companion object {
         ...
         */
        @JvmStatic
        fun bilinear(xKeys: List<Double>, yKeys: List<Double>, values: List<List<Double>>): InterpolatingMap2D =
            InterpolatingMap2D(
